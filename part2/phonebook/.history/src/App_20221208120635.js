...
const addPerson = (event) => {
  ...
  if (alreadyExistingContact) {
    if (window.confirm(
      `${nameObject.name}  already exists, update ${nameObject.name}'s number with new number?`
    )
    ) {
      personsService
        .updateRequest(alreadyExistingContact.id, { ...alreadyExistingContact, number: newNumber })
        .then((responseContact) => {
          setPersons(persons.map((cont) =>
            cont.id !== alreadyExistingContact.id ? cont : responseContact
          )
          );
          notification(
            `${responseContact.name} was updated, new number is now ${responseContact.number}.`
          );
        })
        .catch((error) => {
