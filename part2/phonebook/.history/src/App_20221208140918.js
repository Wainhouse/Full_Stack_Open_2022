...
const App = () => {
  ...
  const addPerson = (event) => {
    ...
    } else if (window.confirm (`${newName.trim()} is already added to phone-book, replace the old number with a new one?`)) {
      personsService
      .update(persons.id, { ...persons, number: newNumber })
      .then((response) => {
        setPersons(
          persons.map((cont) =>
            cont.id !== persons.id ? cont : response
          )
        );
      });
    } else {
